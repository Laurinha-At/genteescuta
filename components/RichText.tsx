...
//
// IMPORTANTE: o editor é "não controlado" — o conteúdo inicial é injetado
// uma vez e o HTML é lido só quando o pai chama getHtml() (ao salvar). Ele
// NÃO dispara estado a cada tecla, então o React não re-renderiza durante a
// digitação e o cursor nunca é resetado (bug que impedia digitar).
// =============================================================
...
export const RichTextEditor = forwardRef<RichHandle, {
  ...
  function cmd(comando: string) {
    // execCommand é depreciado, mas segue funcionando e é o caminho mais
    // leve para um editor interno simples (sem dependências).
    elRef.current?.focus()
  ...
  const Bt = ({ acao, titulo, children }: { acao: string; titulo: string; children: React.ReactNode }) => (
    <button
      type="button"
      title={titulo}
      // onMouseDown com preventDefault preserva a seleção do texto no editor.
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-borda-forte bg-white focus-within:border-marca focus-within:outline focus-within:outline-2 focus-within:outline-offset-[-1px] focus-within:outline-marca">
      ...
      <div
        ...
        style={{ minHeight }}
        className="px-3.5 py-2.5 text-sm leading-6 text-tinta outline-none [&_ol]:list-decimal [&_ol]:pl-5 [&_ul]:list-disc [&_ul]:pl-5 empty:before:text-tinta-3 empty:before:content-[attr(data-placeholder)]"
      />
    </div>
  )
})
